...
static const OptionDef *find_option(const OptionDef *po, const char *name) {
    const char *p = strchr(name, ':');//某个字符首次出现的位置
    int len = p ? p - name : strlen(name);//分割以：间隔的命令

    while (po->name) {//遍历options，找到相同的命令
...
void parse_loglevel(int argc, char **argv, const OptionDef *options) {
    int idx = locate_option(argc, argv, options, "loglevel");//查找是否设置日志
    const char *env;
    ...
    if (idx && argv[idx + 1])
        opt_loglevel(NULL, "loglevel", argv[idx + 1]);//设置日志

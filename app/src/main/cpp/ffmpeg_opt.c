...
//eg. ffmpeg -i xxx.mpg -vcodec libx264 yyy.mkv
int ffmpeg_parse_options(int argc, char **argv) {
    OptionParseContext octx;
    uint8_t error[128];
    int ret;

    memset(&octx, 0, sizeof(octx));//octx指向NULL，为什么不直接用NULL？要开辟内存？

    /* split the commandline into an internal representation FF_ARRAY_ELEMS计算有几组*/
...
#include "cmdutils_common_opts.h"
        ...
        {"f", HAS_ARG | OPT_STRING | OPT_OFFSET |
              OPT_INPUT | OPT_OUTPUT, {.off       = OFFSET(format)}, "force format", "fmt"},
        ...
        {"c", HAS_ARG | OPT_STRING | OPT_SPEC |
              OPT_INPUT | OPT_OUTPUT, {.off       = OFFSET(codec_names)}, "codec name", "codec"},
